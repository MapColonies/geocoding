...
import { withSpan, withSpanSync } from '../../common/tracing';
...
export class LatLonManager {
  ...
  public async latLonToTile({ lat, lon, targetGrid }: WGS84Coordinate & { targetGrid: string }): Promise<GenericGeocodingFeatureResponse> {
    return withSpan(
      LatLonSpanName.MANAGER_TO_TILE,
      {
        attributes: {
          [LatLonAttributes.LAT]: lat,
          [LatLonAttributes.LON]: lon,
          [LatLonAttributes.TARGET_GRID]: targetGrid,
        },
      },
      async (span): Promise<GenericGeocodingFeatureResponse> => {
        ...
        span?.setAttributes({
          [LatLonAttributes.UTM_X]: coordinatesUTM.x,
          [LatLonAttributes.UTM_Y]: coordinatesUTM.y,
          [LatLonAttributes.UTM_ZONE]: coordinatesUTM.zone,
          [LatLonAttributes.TILE_NAME]: tileCoordinateData.tile_name,
        });
  ...
  public latLonToMGRS({
    ...
    return withSpanSync(
      LatLonSpanName.MANAGER_TO_MGRS,
      {
        attributes: {
          [LatLonAttributes.LAT]: lat,
          [LatLonAttributes.LON]: lon,
          [LatLonAttributes.ACCURACY]: accuracy,
          [LatLonAttributes.TARGET_GRID]: targetGrid,
        },
      },
      (span): GenericGeocodingFeatureResponse => {
        ...
        span?.setAttribute(LatLonAttributes.TILE_NAME, mgrsStr);
